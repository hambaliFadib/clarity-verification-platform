...
function mapTestCaseToFormValues(testCase: TestCase): Partial<TestCaseFormValues> {
  return {
    ...
    subModule: (testCase as any).subModule || "Login Flow", // Mock existing
    scenario: (testCase as any).scenario || "Login Scenario", // Mock existing
...
export default function EditTestCasePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <TestCaseForm
      ...
      onCancel={() => router.push(`/test-cases/${id}`)}
