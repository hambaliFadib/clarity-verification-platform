...
export default function PlatformUsersPage() {
  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      setInviteMessage("User added to platform");
    } catch (error: any) {
      setInviteMessage(error.message || "Unable to invite user.");
    } finally {
      setIsInviting(false);
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      <PageHeader
        ...
        subtitle="Manage all users registered across the workspace."
        actions={
      ...
      <div className="bg-white border border-outline-variant rounded-xl p-8 space-y-6 shadow-sm">
        ...
        {inviteMessage && (
          <p className={inviteMessage === "User added to platform" ? "text-body-sm text-emerald-600 font-medium px-2" : "text-body-sm text-error font-medium px-2"}>
            {inviteMessage}
          </p>
        )}
